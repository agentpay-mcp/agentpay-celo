-- Production-only, RPC-scoped wallet onboarding for Celo mainnet.
...
create table if not exists public.setup_runtime_state (
  ...
  chain_id integer not null check (chain_id = 42220),
...
begin
  ...
  if not found or v_runtime.environment <> 'production' or v_runtime.chain_id <> 42220 then
  ...
  insert into public.setup_intents (
    ...
    p_setup_intent_id, lower(p_owner_address), lower(p_executor_address), p_message_to_sign, 'PENDING', 42220,
...
begin
  ...
  values (v_intent.id, v_tenant_id, 'QUEUED', 42220, p_at, p_at)
...
begin
  ...
  if exists (
    ...
    where chain_id = 42220 and deployer_address = lower(p_deployer_address)
  ...
  where chain_id = 42220 and deployer_address = lower(p_deployer_address) and day_key = v_day;
  ...
  insert into public.setup_sponsor_budgets (
    ...
    p_job_id, v_day, 42220, lower(p_deployer_address), p_deployer_nonce, p_gas_limit,
...
begin
  ...
  insert into public.agent_wallets (
    ...
    v_job.tenant_id, v_intent.owner_address, v_intent.predicted_account, 42220,
