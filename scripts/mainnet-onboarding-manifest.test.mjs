...
describe("Celo mainnet onboarding manifest", () => {
  ...
  it("binds complete verified Celo factory evidence without enabling setup or payments", () => {
    ...
    assert.deepEqual(manifest.factory, {
      ...
      executor: deployment.executor.toLowerCase(),
    });
    assert.match(digest, /^[a-f0-9]{64}$/);
  });

  it("uses canonical sorted-key JSON for a stable digest", () => {
    const manifest = boundManifest();
    const reordered = {
  ...
  it("rejects chain, token, route, origin, mode, and partial factory drift", () => {
    const cases = [
      ["chainId", (manifest) => { manifest.chainId = 196; }],
      ["factory.usdc", (manifest) => { manifest.factory.usdc = "0x5555555555555555555555555555555555555555"; }],
      ["account.routeTargets", (manifest) => { manifest.account.routeTargets = [deployment.address]; }],
      ["onboardingOrigin", (manifest) => { manifest.onboardingOrigin = "https://celo.agentpay.site"; }],
  ...
  it("rejects secret-like keys and digest or runtime-artifact mismatch", () => {
    const manifest = structuredClone(boundManifest());
  ...
  it("accepts only the four fail-closed setup modes", () => {
  ...
  it("rejects actor collisions, malformed sponsor limits, and extra deployment metadata", () => {
    assert.throws(
      () => bindVerifiedFactoryDeployment({
