...
describe("README", () => {
  it("describes the implemented local AgentPay runtime instead of stale scaffold state", async () => {
    ...
    assert.match(contents, /skills\/agentpay-celo\/SKILL\.md/);
    assert.match(contents, /explicit.*--runtime|--runtime.*explicit/i);
    assert.match(contents, /npx -y @agentpay-ai\/agentpay-celo@latest install --runtime codex/);
    assert.match(contents, /~\/\.agentpay-celo/);
    assert.match(contents, /MCP (?:key|entry) `agentpay-celo`/i);
    assert.match(contents, /existing X Layer.*untouched|does not overwrite.*X Layer/i);
    ...
    assert.match(contents, /agentpay-celo serve-http/);
  ...
  it("presents the npm CLI as a chat-first install flow", async () => {
    ...
    assert.match(contents, /~\/\.agentpay-celo/);
    assert.match(contents, /agentpay-celo/);
    assert.match(contents, /dedicated HTTPS.*CELO_MAINNET_RPC_URL/i);
    assert.match(contents, /CELO_MAINNET_RPC_FALLBACK_URL=https:\/\/forno\.celo\.org/);
    assert.match(contents, /Celo x402|x402 seller gate/i);
    assert.match(contents, /AGENTPAY_A2MCP_PAYMENT_ENABLED/);
    assert.doesNotMatch(quickStart, /agentpay doctor/i);
    assert.doesNotMatch(quickStart, /agentpay setup-web/i);
