...
describe("publishable AgentPay package manifests", () => {
  ...
  it("keeps the 0.1.21 installer release locked to the auto-switch package chain and isolated skill", async () => {
    ...
    assert.equal(setupWebManifest.dependencies?.["@agentpay-ai/shared-celo"], sharedManifest.version);
    assert.equal(mcpServerManifest.dependencies?.["@agentpay-ai/shared-celo"], sharedManifest.version);
    ...
    assert.equal(lockedSetupWeb?.dependencies?.["@agentpay-ai/shared-celo"], sharedManifest.version);
    assert.equal(lockedMcpServer?.version, mcpServerManifest.version);
    assert.equal(lockedMcpServer?.dependencies?.["@agentpay-ai/shared-celo"], sharedManifest.version);
    assert.equal(lockedShared?.version, sharedManifest.version);
