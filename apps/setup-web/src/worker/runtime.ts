...
const manifestSchema = z.object({
  ...
  onboardingOrigin: z.literal("https://wallet.agentpay.site"),
  factory: z.object({
    ...
    usdc: z.literal(MAINNET_SETUP_USDC),
...
const databaseRuntimeSchema = z.object({
  environment: z.literal("production"), chainId: z.literal(42220),
  setupMode: z.enum(["OFF", "CANARY", "PUBLIC", "DRAIN"]),
...
export interface ProductionSetupWorkerConfig {
  ...
  readonly scopedWorkerTokenPath: string;
...
export function parseProductionSetupWorkerConfig(
  ...
  const scopedWorkerTokenPath = values.AGENTPAY_SETUP_WORKER_TOKEN_PATH!;
  if (!scopedWorkerTokenPath.startsWith("/") || new Set([
    scopedWorkerTokenPath,
    values.AGENTPAY_ONBOARDING_MANIFEST_PATH!,
    values.AGENTPAY_ACCOUNT_RUNTIME_ARTIFACT_PATH!,
  ]).size !== 3) throw new Error("SETUP_WORKER_TOKEN_PATH_INVALID");
  ...
  validateScopedWorkerToken(options.scopedToken, options.nowUnix ?? Math.floor(Date.now() / 1_000));
  ...
  return deepFreeze({
    environment: "production" as const, chainId: 42220 as const, mode,
    scopedWorkerTokenPath, supabaseUrl, supabaseApiKey, mainnetRpcUrl,
...
export async function loadProductionSetupWorkerConfig(
  ...
  const scopedTokenPath = env.AGENTPAY_SETUP_WORKER_TOKEN_PATH?.trim();
  if (!manifestPath || !runtimeArtifactPath || !scopedTokenPath) {
    throw new Error("SETUP_WORKER_CONFIG_MISSING:file paths");
  }
  const [manifestJson, runtimeArtifactJson, scopedToken] = await Promise.all([
    readFile(manifestPath, "utf8"), readFile(runtimeArtifactPath, "utf8"),
    readProductionSetupWorkerScopedToken(scopedTokenPath),
  ]);
  return parseProductionSetupWorkerConfig(env, { manifestJson, runtimeArtifactJson, scopedToken });
}

export async function readProductionSetupWorkerScopedToken(
  path: string,
  ...
  try {
    ...
    validateScopedWorkerToken(token, nowUnix);
    return token;
  } catch {
...
export async function createProductionSetupWorkerRuntime(
  ...
  const tokenProvider = () => readProductionSetupWorkerScopedToken(config.scopedWorkerTokenPath);
  ...
  await verifyWorkerRuntime(config, provider, signer, factory, fetchImplementation, tokenProvider);
  ...
  const worker = createSetupDeploymentWorker({
    ...
    verifyPreflight: async (claim) => verifyClaimPreflight(
      config, provider, signer, factory, claim, fetchImplementation, tokenProvider,
    ),
...
async function verifyWorkerRuntime(
  ...
  const [network, code, executor, usdc, policyVersion, creationHash, balance, database] = await Promise.all([
    provider.getNetwork(), provider.getCode(config.factoryAddress), factory.executor(), factory.USDC(),
    factory.POLICY_VERSION(), factory.accountCreationCodeHash(), provider.getBalance(signer.address),
    readWorkerDatabaseRuntime(config, fetchImplementation, tokenProvider),
  ]);
  assertProductionSetupWorkerChainId(network.chainId);
  if (code === "0x" || keccak256(code).toLowerCase() !== config.factoryRuntimeCodeHash
    || String(executor).toLowerCase() !== config.manifest.factory.executor.toLowerCase()
    || String(usdc).toLowerCase() !== MAINNET_SETUP_USDC.toLowerCase()
...
export function assertProductionSetupWorkerChainId(chainId: bigint): void {
  if (chainId !== BigInt(MAINNET_SETUP_CHAIN_ID)) {
    throw new Error("SETUP_WORKER_RUNTIME_MISMATCH");
  }
}
...
async function verifyClaimPreflight(
  ...
  if (ownerCode !== "0x" || claim.homeChainId !== 42220
...
function createVerificationReader(provider: JsonRpcProvider): SetupAccountVerificationReader {
  return Object.freeze({
    ...
    async getAccountState(address: string) {
      ...
      const [owner, executor, paused, domainSeparator, allowedUsdc, allowedUsdt] = await Promise.all([
        account.owner(), account.executor(), account.paused(), account.domainSeparator(),
        account.allowedTokens(MAINNET_SETUP_USDC), account.allowedTokens("0x48065fbbe25f71c9282ddf5e1cd6d6a887483d5e"),
      ]);
      return { owner, executor, paused, domainSeparator, allowedUsdc, allowedUsdt };
    },
...
async function readWorkerDatabaseRuntime(
  ...
  const response = await fetchImplementation(`${config.supabaseUrl}/rest/v1/rpc/read_production_setup_worker_runtime_state`, {
    method: "POST",
    headers: { apikey: config.supabaseApiKey, authorization: `Bearer ${token}`, "content-type": "application/json" },
    body: "{}",
  });
...
function createFactory(config: ProductionSetupWorkerConfig, provider: JsonRpcProvider): Contract {
  return new Contract(config.factoryAddress, [
    "function executor() view returns (address)", "function USDC() view returns (address)",
