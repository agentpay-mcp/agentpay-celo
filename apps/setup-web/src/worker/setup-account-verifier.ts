...
export async function verifySetupAccount(input: {
  ...
  if (chainId !== 42220) throw new Error("SETUP_CHAIN_MISMATCH");
  ...
  const factoryEvent = findFactoryReceiptEvent(factoryLogs, input.claim, input.receipt);
  let deploymentBlockNumber = factoryEvent.log.blockNumber;
  if (input.receipt && factoryEvent.kind === "AccountDeployed"
    && deploymentBlockNumber !== input.receipt.blockNumber) throw new Error("SETUP_FACTORY_EVENT_MISMATCH");
  if (factoryEvent.kind === "AccountReused") {
    const historicalFactoryLogs = await fetchSetupLogsInChunks(
      input.reader,
      input.claim.factoryAddress,
      input.factoryDeploymentBlock,
      factoryEvent.log.blockNumber,
    );
    deploymentBlockNumber = findOriginalFactoryDeployment(historicalFactoryLogs, input.claim).blockNumber;
  }
...
function findFactoryReceiptEvent(
  logs: readonly SetupVerificationLog[],
  claim: SetupWorkerClaim,
  receipt?: SetupDeploymentReceipt,
): Readonly<{ kind: "AccountDeployed" | "AccountReused"; log: SetupVerificationLog }> {
  const matching = logs.flatMap((log) => {
    if (receipt && log.transactionHash.toLowerCase() !== receipt.transactionHash.toLowerCase()) return [];
    try {
      const parsed = factoryInterface.parseLog({ topics: [...log.topics], data: log.data });
      if (!parsed || (parsed.name !== "AccountDeployed" && parsed.name !== "AccountReused")) return [];
      if (parsed.name === "AccountReused" && !receipt) return [];
      if (String(parsed.args.owner).toLowerCase() !== claim.ownerAddress.toLowerCase()
        || String(parsed.args.account).toLowerCase() !== claim.predictedAccount.toLowerCase()) return [];
      if (parsed.name === "AccountDeployed"
        && String(parsed.args.salt).toLowerCase() !== claim.deploymentSalt.toLowerCase()) {
        return [];
      }
      if (receipt && String(parsed.args.authorizationHash).toLowerCase() !== claim.authorizationHash.toLowerCase()) return [];
      return [{ kind: parsed.name, log } as const];
    } catch {
      return [];
    }
  });
...
function findOriginalFactoryDeployment(
  logs: readonly SetupVerificationLog[],
  claim: SetupWorkerClaim,
  ...
  const matching = logs.filter((log) => {
    try {
      const parsed = factoryInterface.parseLog({ topics: [...log.topics], data: log.data });
      return parsed?.name === "AccountDeployed"
        && String(parsed.args.owner).toLowerCase() === claim.ownerAddress.toLowerCase()
        && String(parsed.args.account).toLowerCase() === claim.predictedAccount.toLowerCase()
        && String(parsed.args.salt).toLowerCase() === claim.deploymentSalt.toLowerCase();
    } catch {
      return false;
    }
...
function assertSafeAccountHistory(logs: readonly SetupVerificationLog[], expectedExecutor: string): void {
  for (const log of logs) {
    ...
    if (parsed.name === "TokenAllowedUpdated") {
      ...
      if ((token !== MAINNET_SETUP_USDC.toLowerCase() && allowed)
        || (token === MAINNET_SETUP_USDC.toLowerCase() && !allowed)) {
        throw new Error("SETUP_ACCOUNT_POLICY_MISMATCH");
      }
    }
