...
function claim(status: SetupWorkerClaim["jobStatus"] = "SIGNING", extra: Partial<SetupWorkerClaim> = {}): SetupWorkerClaim {
  const authorization = {
    ...
    accountCreationCodeHash: hash("5"), accountRuntimeCodeHash: hash("6"), token: MAINNET_SETUP_USDC,
  ...
  return Object.freeze({
    ...
    homeChainId: 42220, deploymentNonce: authorization.deploymentNonce, manifestSha256: authorization.manifestSha256,
...
describe("recoverable setup deployment worker", () => {
  ...
  it("refuses to sign or rebroadcast after the owner authorization expires", async () => {
    for (const status of ["SIGNING", "SIGNED"] as const) {
      const admitted = claim(status);
      const events: string[] = [];
      const worker = createSetupDeploymentWorker(dependencies(admitted, events));

      const result = await worker.processNext(admitted.expiresAt);
