...
export function createSetupDeploymentWorker(dependencies: SetupDeploymentWorkerDependencies) {
  ...
  async function processClaim(claim: SetupWorkerClaim, at: string): Promise<SetupDeploymentWorkerResult> {
    if (["SIGNING", "SIGNED"].includes(claim.jobStatus) && !authorizationIsLive(claim.expiresAt, at)) {
      return manualReview(claim, at, "SETUP_AUTHORIZATION_EXPIRED");
    }
...
function assertPersistedTransaction(
  ...
  if (parsed.type !== 2 || parsed.chainId !== 42220n || !parsed.from || !parsed.to
...
function authorizationIsLive(expiresAt: string, at: string): boolean {
  const expiration = Date.parse(expiresAt);
  const current = Date.parse(at);
  return Number.isFinite(expiration) && Number.isFinite(current) && current < expiration;
}
