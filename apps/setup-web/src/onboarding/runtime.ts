...
const ONBOARDING_ORIGIN = "https://wallet.agentpay.site" as const;
const ONBOARDING_HOST = "wallet.agentpay.site" as const;
const POLICY_VERSION = "0x7ca42c75d0d0ce25c514495482839ca84b4d4e3e445080004653e98bdebeb16c";
const TOKEN_ALLOWLIST_HASH = "0xcbb6c89bb5bf981f61dac2e79e732d017d0aefcdb843f6202c5960618184fe67";
...
export interface ProductionOnboardingConfig {
  ...
  readonly scopedWebTokenPath: string;
...
export function parseProductionOnboardingConfig(
  ...
  const required = [
    "AGENTPAY_ENVIRONMENT", "AGENTPAY_SETUP_MODE", "AGENTPAY_SETUP_WEB_TOKEN_PATH", "SUPABASE_URL",
    "SUPABASE_PUBLISHABLE_KEY",
    "CELO_MAINNET_RPC_URL", "AGENTPAY_ONBOARDING_MANIFEST_PATH",
  ...
  const scopedWebTokenPath = normalized.AGENTPAY_SETUP_WEB_TOKEN_PATH!;
  if (!scopedWebTokenPath.startsWith("/") || new Set([
    scopedWebTokenPath,
    normalized.AGENTPAY_ONBOARDING_MANIFEST_PATH!,
    normalized.AGENTPAY_ACCOUNT_RUNTIME_ARTIFACT_PATH!,
  ]).size !== 3) throw new Error("SETUP_TOKEN_PATH_INVALID");
  ...
  const mainnetRpcUrl = requireProductionUrl(normalized.CELO_MAINNET_RPC_URL!, "rpc");
  ...
  validateScopedToken(options.scopedToken, options.nowUnix ?? Math.floor(Date.now() / 1_000));
...
export async function verifyProductionOnboardingRuntime(
  ...
  try {
    ...
    if (
      ...
      !equalAddress(identity.usdc, MAINNET_SETUP_USDC) ||
...
export async function loadProductionOnboardingConfig(
  ...
  const scopedTokenPath = env.AGENTPAY_SETUP_WEB_TOKEN_PATH?.trim();
  if (!manifestPath || !runtimeArtifactPath || !scopedTokenPath) {
    throw new Error("SETUP_CONFIG_MISSING:file paths");
  }
  const [manifestJson, runtimeArtifactJson, scopedToken] = await Promise.all([
    readFile(manifestPath, "utf8"),
    readFile(runtimeArtifactPath, "utf8"),
    readProductionOnboardingScopedToken(scopedTokenPath),
  ]);
  return parseProductionOnboardingConfig(env, { manifestJson, runtimeArtifactJson, scopedToken });
}

export async function readProductionOnboardingScopedToken(
  path: string,
  ...
  try {
    ...
    validateScopedToken(token, nowUnix);
    return token;
  } catch {
    throw new Error("SETUP_SCOPED_TOKEN_INVALID");
  }
}
...
export async function createProductionOnboardingRuntime(
  ...
  const tokenProvider = () => readProductionOnboardingScopedToken(config.scopedWebTokenPath);
  ...
  const readiness = createEthersReadiness(config, provider, contract, options.fetch ?? fetch, tokenProvider);
...
function createEthersReadiness(
  ...
  return {
    ...
    async getFactoryIdentity() {
      const [executorAddress, usdc, policyVersion, accountCreationCodeHash] = await Promise.all([
        contract.executor(), contract.USDC(), contract.POLICY_VERSION(), contract.accountCreationCodeHash(),
      ]);
      return { executorAddress, usdc, policyVersion, accountCreationCodeHash } as const;
    },
