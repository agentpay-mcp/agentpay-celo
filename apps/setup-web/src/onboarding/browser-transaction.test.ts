...
describe("bounded onboarding JSON", () => {
  it("accepts JSON within the limit and rejects before buffering an oversized declared body", async () => {
    const small = new Request("https://wallet.agentpay.site/celo/setup/api/challenge", {
      method: "POST",
    ...
    const oversized = new Request("https://wallet.agentpay.site/celo/setup/api/challenge", {
      method: "POST",
      headers: {
  ...
  it("rejects non-JSON, malformed JSON, and streamed bodies over 4 KiB", async () => {
    await assert.rejects(
      () => readBoundedJsonBody(new Request("https://wallet.agentpay.site", {
        method: "POST",
