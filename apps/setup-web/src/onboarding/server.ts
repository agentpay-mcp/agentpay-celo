...
export interface ProductionOnboardingDependencies {
  ...
  readonly origin: "https://wallet.agentpay.site";
  readonly host: "wallet.agentpay.site";
...
const routes = Object.freeze(new Map([
  ["/celo/setup", "GET"],
  ["/celo/setup/api/challenge", "POST"],
  ["/celo/setup/api/status", "GET"],
  ["/celo/setup/api/authorize", "POST"],
  ["/celo/setup/healthz", "GET"],
  ["/celo/setup/readyz", "GET"],
]));
...
export function createProductionOnboardingHandler(dependencies: ProductionOnboardingDependencies) {
  ...
  return async (request: Request): Promise<Response> => {
    ...
    if (path === "/celo/setup/healthz") return json({ status: "ok", mode: dependencies.mode }, 200);
    if (path === "/celo/setup/readyz") {
      try {
    ...
    try {
      ...
      if (path === "/celo/setup/api/challenge") return await handleChallenge(request, dependencies);
      if (path === "/celo/setup/api/status") return await handleStatus(request, dependencies);
...
function assertDependencies(dependencies: ProductionOnboardingDependencies): void {
  if (dependencies.origin !== "https://wallet.agentpay.site" || dependencies.host !== "wallet.agentpay.site") {
...
export async function startProductionOnboardingServer(
  ...
  return Object.freeze({
    url: `http://${options.hostname ?? "127.0.0.1"}:${address.port}/celo/setup`,
