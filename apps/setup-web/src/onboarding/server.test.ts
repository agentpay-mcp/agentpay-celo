...
const origin = "https://wallet.agentpay.site";
const host = "wallet.agentpay.site";
    ...
    policy: {
      async derive(input) {
        ...
        const typedData = createMainnetWalletSetupTypedData({
          ...
          tokenAllowlistHash: "0xcbb6c89bb5bf981f61dac2e79e732d017d0aefcdb843f6202c5960618184fe67",
...
async function createChallenge(fixture = createFixture()) {
  const response = await fixture.handler(new Request(`${origin}/celo/setup/api/challenge`, {
    method: "POST",
    headers: apiHeaders(),
...
describe("production onboarding route and browser boundary", () => {
  it("exposes only the six exact routes and applies hardened no-store headers without CORS", async () => {
    ...
    const page = await handler(new Request(`${origin}/celo/setup`, {
    ...
    for (const path of ["/", "/review", "/api/setup", "/celo/setup/api/challenge/", "/favicon.ico"]) {
      const response = await handler(new Request(`${origin}${path}`, { headers: { host } }));
      assert.equal(response.status, 404, path);
    }
    const wrongMethod = await handler(new Request(`${origin}/celo/setup/api/status`, {
  ...
  it("rejects wrong Host, Origin, Fetch Metadata, proxy identity, and unavailable rate limiter", async () => {
    ...
    for (const invalid of invalidHeaders) {
      const { handler } = createFixture();
      const response = await handler(new Request(`${origin}/celo/setup/api/challenge`, {
    ...
    const limitedResponse = await limited.handler(new Request(`${origin}/celo/setup/api/challenge`, {
      method: "POST", headers: apiHeaders(), body: JSON.stringify({ ownerAddress: owner.address }),
    }));
...
describe("production onboarding challenge, authorization, and status", () => {
  it("derives and stores all policy fields server-side, returning capability once with a hardened cookie", async () => {
    ...
    assert.equal(body.typedData.message.homeChainId, 42220);
    assert.equal(body.typedData.message.token, MAINNET_SETUP_USDC);
  ...
  it("requires the bound cookie, capability, and CSRF then admits the valid owner signature idempotently", async () => {
    ...
    const authorizationRequest = (csrfToken = challenge.body.csrfToken, capability = challenge.body.capability) =>
      new Request(`${origin}/celo/setup/api/authorize`, {
        method: "POST",
        headers: apiHeaders({
          cookie,
  ...
  it("does not consume admission for a wrong signature and returns only sanitized status", async () => {
    ...
    const wrong = await challenge.fixture.handler(new Request(`${origin}/celo/setup/api/authorize`, {
    ...
    const statusResponse = await challenge.fixture.handler(new Request(`${origin}/celo/setup/api/status`, {
  ...
  it("keeps health/status available but refuses new challenges in OFF and DRAIN", async () => {
    for (const mode of ["OFF", "DRAIN"] as const) {
      ...
      const health = await handler(new Request(`${origin}/celo/setup/healthz`, { headers: { host } }));
      assert.equal(health.status, 200);
      const challenge = await handler(new Request(`${origin}/celo/setup/api/challenge`, {
        method: "POST", headers: apiHeaders(), body: JSON.stringify({ ownerAddress: owner.address }),
      }));
