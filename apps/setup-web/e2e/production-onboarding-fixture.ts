...
const origin = "https://wallet.agentpay.site" as const;
const host = "wallet.agentpay.site" as const;
...
export async function createProductionOnboardingE2eFixture(
  ...
  function createWorker() {
    return createSetupDeploymentWorker({
      ...
      async verifyPreflight(claim) {
        if (claim.homeChainId !== 42220 || claim.factoryAddress.toLowerCase() !== factoryAddress.toLowerCase()
  ...
  async function start(requestedPort = 0) {
    server = createServer(async (incoming, outgoing) => {
      try {
        ...
        if (path.startsWith("/celo/setup/api/")) {
  ...
  return {
    ...
    stores,
    broadcastAttempts,
    get url() { return `http://localhost:${port}/celo/setup`; },
    chatEntryState() {
      ...
      return activeWallet || completed
        ...
        : { status: 409, state: "AGENTPAY_SETUP_REQUIRED" as const, setupUrl: `${origin}/celo/setup` };
    },
