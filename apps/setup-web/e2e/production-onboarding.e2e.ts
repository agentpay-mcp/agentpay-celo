...
test("production onboarding completes the setup-required chat handoff and sponsored deployment", async ({ page }) => {
  expect(fixture.chatEntryState()).toEqual({
    status: 409,
  ...
  await expect(page.locator("#details")).toContainText("42220");
  ...
  expect(fixture.chatEntryState()).toEqual({ status: 200, state: "OAUTH_READY" });
...
test("production onboarding lets an existing active user bypass sponsorship", async () => {
  ...
  try {
    expect(existing.chatEntryState()).toEqual({ status: 200, state: "OAUTH_READY" });
...
test("production onboarding fails closed on expiry, lost response, and reload without capability", async ({ page }) => {
  ...
  try {
    ...
    await lostPage.route("**/celo/setup/api/challenge", async (route) => {
...
test("production onboarding enforces OFF, DRAIN, rate-limit, and manual-review states", async ({ page }) => {
  ...
  try {
    ...
    expect(review.chatEntryState().state).toBe("AGENTPAY_SETUP_REQUIRED");
  } finally {
