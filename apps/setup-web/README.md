...
Browser wallet flows request the server-bound Celo chain before signing. They call `wallet_switchEthereumChain`, fall back to `wallet_addEthereumChain` with pinned official network metadata on error `4902`, and fail closed when the owner rejects or the wallet remains on the wrong chain.
