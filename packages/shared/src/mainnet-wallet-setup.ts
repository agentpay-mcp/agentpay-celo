...
export const MAINNET_SETUP_CHAIN_ID = 42220 as const;
export const MAINNET_SETUP_ENVIRONMENT = "production" as const;
export const MAINNET_SETUP_USDC = "0xcebA9300f2b948710d2653dD7B07f33A8B32118C" as const;
export const MAINNET_ONBOARDING_URL = "https://wallet.agentpay.site/celo/setup" as const;
export const MAINNET_SETUP_TOKEN_ALLOWLIST_HASH =
  "0xcbb6c89bb5bf981f61dac2e79e732d017d0aefcdb843f6202c5960618184fe67" as const;
...
export const mainnetWalletSetupMessageSchema = z
  .object({
    ...
    token: z.literal(MAINNET_SETUP_USDC),
  ...
  .superRefine((message, context) => {
    const actors = [message.owner, message.executor, message.factory].map((address) => address.toLowerCase());
...
export const MAINNET_WALLET_SETUP_TYPES = Object.freeze({
  MainnetWalletSetup: Object.freeze(mainnetWalletSetupTypeFields.map((field) => Object.freeze({ ...field }))),
});
...
export const mainnetWalletSetupChallengeRequestSchema = z.object({ ownerAddress: nonzeroEvmAddressSchema }).strict();
...
const publicCodeSchema = z.string().min(1).max(64).regex(/^[A-Z][A-Z0-9_]*$/, "Expected a stable uppercase public code");
